...
vec3 R_norm(vec3 direction) {
    direction = normalize(direction);
    return 2 * normal * dot(normal, direction) - direction;
}

vec3 specular(vec3 direction) {
    vec3 point_view_direction = normalize(camera_position - position);
    float power = 1 / (roughness * roughness) - 1;
    return glossiness * albedo * pow(max(0.0, dot(R_norm(direction), point_view_direction)), power);
}
...
void main()
{
    ...
    vec3 norm_sun_direction = normalize(sun_direction);

    vec3 sun_color = (diffuse(norm_sun_direction) + specular(norm_sun_direction)) * sun_color;

    float distance = distance(position, point_light_position);

    vec3 point_light_direction = normalize(point_light_position - position);

    vec3 point_color = ((diffuse(point_light_direction)) + specular(point_light_direction))
        * 1 / (point_light_attenuation.x + point_light_attenuation.y * distance + point_light_attenuation.z * distance * distance)
        * point_light_color;

    vec3 color = ambient_color + sun_color + point_color;
...
int main() try {
    ...
    while (running) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            view = glm::translate(view, {float(i / 3 - 1) * 3, float(i % 3 - 1) * 3, -camera_distance});
            ...
            glUniform3f(sun_direction_location, 1, 1, 1);
            glUniform3f(sun_color_location, 0.0f, 1.f, 1.f);

            glUniform3f(point_light_position_location, 2.f * cos(time), 2.f * sin(time), -1.f);
            glUniform3f(point_light_color_location, 1.f, 1.f, 0.f);
            glUniform3f(point_light_attenuation_location, 1.0, 0.0, 0.01);

            glUniform1f(glossiness_location, 5.0 * float(i / 3) );
            glUniform1f(roughness_location, 0.2 * float(i % 3));
